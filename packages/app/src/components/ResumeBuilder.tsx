...
const ResumeBuilder: React.FC = () => {
  ...
  const [pdfData, setPdfData] = useState<
    { [key: string]: { text: string; images: string[]; metadata: any; optimized?: string; previewUrl?: string } }
  >({})
  ...
  const saveDraft = () => {
    ...
    try {
      ...
      setTimeout(() => {
        setIsSaving(false)
        const content = `${resumeTemplate.header}\n\n${resumeTemplate.sidebar}\n\n${resumeTemplate.mainContent}`
        saveResumeDraft(`Resume Draft ${new Date().toISOString().split('T')[0]}`, content, selectedTemplate || 'modern')
      }, 500)
    } catch (error) {
                                ...
                                <div className='mt-2'>
                                  ...
                                  {opt.length > 400 && (
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation()
                                        setExpandedOptimized((prev) => ({ ...prev, [file.name]: !prev[file.name] }))
                                      }}
                                      className='mt-2 text-xs underline text-blue-600 dark:text-blue-300'
                                    >
              ...
              <div className='mt-12 text-center'>
                ...
                {hasSelectedMode && resumeMode === 'scratch' && (
                  <div className='max-w-4xl mx-auto mt-8'>
                    <div
                      className='bg-white dark:bg-gray-100 shadow-2xl border border-gray-300 dark:border-gray-400 min-h-[800px] p-8 relative'
                  ...
                  <div className='max-w-4xl mx-auto mt-8'>
                    ...
                    <div className='mt-6 p-4 bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 rounded-lg'>
                      <p className='text-sm text-blue-800 dark:text-blue-200'>
                        💡 <strong>Tip:</strong>{' '}
                        Your changes are auto-saved locally. Click "Save Draft" to ensure your work is saved, then
                        "Download PDF" to get your formatted resume.
                      </p>
                    </div>
                  </div>
                )}
