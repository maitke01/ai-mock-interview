...
import { useResumeStore } from '../stores/resumeCompletionStore'
...
const ClassicPreview: React.FC<
  { data: { header: string; sidebar: string; mainContent: string }; onChange: (k: string, v: string) => void }
> = ({ data, onChange }) => {
  return (
    <div className='bg-gray-50 p-6 rounded shadow max-w-[680px] mx-auto border border-gray-200'>
      ...
      <div className='flex gap-6 mt-4'>
        <div
          className='w-1/3 text-sm border-r pr-4 text-gray-800 dark:text-gray-900'
          contentEditable
          suppressContentEditableWarning
        ...
        <div
          className='flex-1 text-sm text-gray-800 dark:text-gray-900'
          contentEditable
          suppressContentEditableWarning
...
const EditableTemplateEditor: React.FC<Props> = ({ resume, suggestion, onSave }) => {
  ...
  const submitForAIFormatting = async () => {
    ...
    try {
      ...
      saveDraft() // This is the local saveDraft function
    }
  }
    ...
    <div className='space-y-3'>
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-2'>
          ...
          <button className='text-sm px-2 py-1 bg-gray-200 text-gray-800 rounded' onClick={handleDownload}>
            Download
          </button>
        </div>
      </div>
        ...
        <div>
          <div className='flex items-center gap-4 mb-3'>
            <div className='flex items-center gap-2'>
              <button
                className={`border rounded p-1 ${templateType === 'modern' ? 'ring-2 ring-blue-500' : ''}`}
                onClick={() => selectTemplate('modern')}
              >
                <img src={modernPreview} alt='modern preview' className='h-12 w-16 object-cover' />
              </button>
              <button
                className={`border rounded p-1 ${templateType === 'classic' ? 'ring-2 ring-blue-500' : ''}`}
            ...
            <div className='ml-auto flex gap-2'>
              <button className='text-sm px-3 py-1 bg-yellow-500 text-white rounded' onClick={saveDraft}>
                Save Draft
              </button>
              <button className='text-sm px-3 py-1 bg-indigo-600 text-white rounded' onClick={submitForAIFormatting}>
                Submit for AI Formatting
              </button>
              <button className='text-sm px-3 py-1 bg-gray-800 text-white rounded' onClick={generatePdfPreview}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  a.download = resume?.fileName
                    ? `${resume.fileName.replace(/\.[^/.]+$/, '')}-preview.pdf`
                    : 'resume-preview.pdf'
                  document.body.appendChild(a)
                  a.click()
                  a.remove()
                ...
                <>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      a.download = resume?.fileName
                        ? `${resume.fileName.replace(/\.[^/.]+$/, '')}-optimized.pdf`
              ...
              {resume?.previewUrl && (
                <>
                  <button
                    className='text-sm px-3 py-1 bg-rose-600 text-white rounded'
                    onClick={() => {
                      setPreviewMode('original')
                      setOptimizedPreviewUrl(null)
                    }}
                  ...
                  <button
                    className='text-sm px-3 py-1 bg-gray-200 text-gray-900 rounded'
                    onClick={() => {
                      const a = document.createElement('a')
                      a.href = String(resume.previewUrl)
                      a.download = resume?.fileName
                        ? `${resume.fileName.replace(/\.[^/.]+$/, '')}-original.pdf`
                        : 'resume-original.pdf'
                      document.body.appendChild(a)
                      a.click()
                      a.remove()
                ...
                ? (
                  resume?.previewUrl
                    ? (
                      ...
                      >
                        <p>
                          PDF preview not supported. <a href={String(resume.previewUrl)}>Open PDF</a>
                        </p>
                      </object>
                    )
                  ...
                  optimizedPreviewUrl
                    ? (
                      <object
                        ...
                        width='100%'
                        height={600}
                        aria-label='Optimized PDF preview'
                      >
                        <p>
                          PDF preview not supported. <a href={optimizedPreviewUrl}>Open PDF</a>
                        </p>
                      </object>
                    )
                ...
                : (
                  previewUrl
                    ? (
                      ...
                      >
                        <p>
                          PDF preview not supported. <a href={previewUrl}>Open PDF</a>
                        </p>
                      </object>
                    )
                    : (
                      <div className='text-xs text-gray-500'>
                        No preview yet. Edit template and click Render Preview.
                      </div>
                    )
                )}
            </div>
