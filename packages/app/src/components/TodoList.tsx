...
const TodoList: React.FC<TodoListProps> = ({ onWidthChange }) => {
  const { tasks, addTask, toggleTask, deleteTask, clearCompleted, isSidebarOpen, setSidebarOpen, fetchTodos, isLoading, error } = useTodos()
  ...
  // Fetch todos from server on mount
  useEffect(() => {
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isLoading && tasks.length === 0 ? (
            <div className="text-center mt-8">
              <svg className="w-8 h-8 mx-auto text-blue-500 animate-spin mb-4" fill="none" viewBox="0 0 24 24">
