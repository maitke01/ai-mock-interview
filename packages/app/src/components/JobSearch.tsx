...
const JobSearch: React.FC = () => {
  ...
  const handleSearch = (openApply = false) => {
    try {
      if (openApply && jobUrl && jobUrl.trim()) {
        window.open(jobUrl.trim(), '_blank')
        return
      }
      const q = encodeURIComponent(String(query || '').trim())
      const l = encodeURIComponent(String(location || '').trim())
      let url = `https://www.indeed.com/jobs?q=${q}`
      if (l) url += `&l=${l}`
      window.open(url, '_blank')
    } catch (e) {
      console.warn('Search failed', e)
      setPopupMessage('Failed to open Indeed. Please try again.')
      setShowPopup(true)
    }
  }
                ...
                <div>
                  ...
                  <div className='flex flex-col md:flex-row md:items-start gap-3'>
                    <input
                      ...
                      className='flex-1 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400'
                    />
                    <input
                      type='text'
                      value={location}
                      ...
                      className='w-full md:w-72 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400'
                    />
                    <div className='flex items-center gap-2 w-full md:w-auto'>
                      <button
                        type='submit'
                        className='mt-0 bg-gradient-to-r from-pink-600 to-pink-700 hover:from-pink-700 hover:to-pink-800 text-white px-6 py-2 rounded-full font-medium transition-colors w-full md:w-44 border-2 border-transparent'
                      >
                        Search Jobs
                      </button>
                    </div>
        ...
        <div className='fixed inset-0 bg-black/40 flex items-center justify-center z-50'>
          <div className='bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 text-center'>
            ...
            <div className='flex items-center justify-center gap-4'>
              ...
              <button className='px-4 py-2 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 hover:text-red-600 dark:hover:text-red-400 rounded-md font-medium transition-colors' onClick={() => setShowConfirm(false)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
