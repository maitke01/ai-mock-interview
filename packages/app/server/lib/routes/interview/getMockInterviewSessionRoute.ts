...
export const getMockInterviewSessionRoute: Route<'/api/mock-interview-session/:sessionId'> = async (ctx) => {
  ...
  try {
    ...
    // Get session details
    const result = await durableMockInterview.getSession(sessionId)

    return ctx.json(result)
  } catch (error) {
    console.error('Error in getMockInterviewSessionRoute:', error)
    return ctx.json({
