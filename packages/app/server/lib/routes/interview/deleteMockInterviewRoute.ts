...
export const deleteMockInterviewRoute: Route = async (ctx) => {
  ...
  try {
    ...
    const result = await durableAccount.deleteMockInterview(parseInt(interviewId))

    return ctx.json(result)
  } catch (error) {
    console.error('Error in deleteMockInterviewRoute:', error)
