...
export const getSessionFeedbackRoute: Route<'/api/mock-interview-session/:sessionId/feedback'> = async (ctx) => {
  ...
  try {
    ...
    // Get session feedback scores
    const result = await durableMockInterview.getSessionFeedback(sessionId)

    return ctx.json(result)
  } catch (error) {
    console.error('Error in getSessionFeedbackRoute:', error)
    return ctx.json({
