...
export const getMockInterviewRoute: Route = async (ctx) => {
  ...
  try {
    ...
    const result = await durableAccount.getMockInterview(parseInt(interviewId))

    return ctx.json(result)
  } catch (error) {
    console.error('Error in getMockInterviewRoute:', error)

    return ctx.json({
      error: 'Failed to get mock interview',
